...
class Q_Image_ICO {
	 ...
	 *
	 * If the constructor is not passed a file, a file will need to be supplied using the addImage
	...
	function __construct( $file = false, $sizes = array() ) {
		...
		foreach ( $requiredFunctions as $function ) {
			if ( ! function_exists( $function ) ) {
				trigger_error( "The Q_ICO class was unable to find the $function function, which is part of the GD library. Ensure that the system has the GD library installed and that PHP has access to it through a PHP interface, such as PHP's GD module. Since this function was not found, the library will be unable to create ICO files." );
				return;
			}
		}
	 ...
	 */
	function addImage( $file, $sizes = array() ) {
		if ( ! $this->$hasRequirements )
			return false;

		if ( false === ( $im = $this->_loadImageFile( $file ) ) )
			return false;
	 ...
	 */
	private function _loadImageFile( $file ) {
