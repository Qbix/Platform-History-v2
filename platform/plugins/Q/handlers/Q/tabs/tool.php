 ...
 *  @param {string} [$options.attributes] An associative array of the form name => attributes array, for adding attributes to tabs
...
function Q_tabs_tool($options)
{
	...
	foreach ($tabs as $name => $title) {
		$url = Q::ifset($urls, $name, null);
		if ($name === $sel
		or $name === $uri_string
		or $url === $uri_string
		or $name === $uri_string2
		or $url === $uri_string2
		or $url === Q_Request::url()) {
			$selectedName = $name;
			break;
		}
	}
	...
	Q_Response::addStylesheet('{{Q}}/css/tools/tabs.css', 'Q');
