 ...
 *   @param {Q.Event} [options.onRefresh] Return false from this event to cancel refresh
 */
Q.Tool.define('Q/countdown', function () {
	if (!this.state.timestamp) {
		var attribute = this.getAttribute("data-timestamp");
		if (!attribute) {
			throw new Q.Error("Q/countdown: timestamp not specified");
		}
		this.state.timestamp = parseInt(attribute);
	}
	this.ival = setInterval(function () {
		var data = Q.timeRemaining(timestamp * 1000);
		for (var k in data) {
