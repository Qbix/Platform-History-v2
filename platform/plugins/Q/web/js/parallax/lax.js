...
(function () {
  var lax = function () {
    ...
    lax.createLaxObject = function (el) {
      ...
      var optionString = el.getAttribute('data-lax-options');
      if (optionString) {
		  optionString.split(" ").forEach(function (str) {
			  var _o$split = str.split("="),
				  _o$split2 = _slicedToArray(_o$split, 2),
				  key = _o$split2[0],
				  val = _o$split2[1];

			  o.options[key] = fnOrVal(val);
		  });
	  }
    ...
    lax.updateElement = function (o) {
      var originalStyle = o.originalStyle,
          ...
          el = o.el;
      var r = typeof anchorTop === 'number' ? anchorTop - lastY : lastY;
      var style = {
