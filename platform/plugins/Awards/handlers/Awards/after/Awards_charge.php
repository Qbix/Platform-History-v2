...
function Awards_after_Awards_charge($params)
{
	...
	if ($stream) {
		...
		if ($stream->type === 'Awards/subscription') {
	...
	$link = Q_Request::baseUrl('action.php')
		. "/Awards/payment?publisherId=$publisherId&userId=".$user->id;
	...
	if ($user->emailAddress) {
		...
		$email->retrieve(true);
		$emailSubject = Q_Config::get('Awards', 'transactional', 'charged', 'subject', false);
		$emailView = Q_Config::get('Awards', 'transactional', 'charged', 'body', false);
		...
		if ($mobileView = Q_Config::get('Awards', 'transactional', 'charged', 'sms', false)) {
	...
	if ($publisher->emailAddress) {
		...
		$email->retrieve(true);
		$emailSubject = Q_Config::get('Awards', 'transactional', 'charge', 'subject', false);
		$emailView = Q_Config::get('Awards', 'transactional', 'charge', 'body', false);
		if ($emailSubject !== false and $emailView) {
			$email->sendMessage($emailSubject, $emailView, $fields);
		}
		...
		$mobile->retrieve(true);
		if ($mobileView = Q_Config::get('Awards', 'transactional', 'charge', 'sms', false)) {
			$mobile->sendMessage($mobileView, $fields);
		}
	}
}