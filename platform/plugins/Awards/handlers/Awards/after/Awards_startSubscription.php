...
function Awards_after_Awards_startSubscription($params)
{
	...
	$link = Q_Request::baseUrl('action.php')
		. "/Awards/subscription?publisherId=$publisherId&userId=".$user->id;
	...
	if ($user->emailAddress) {
		...
		$email->retrieve(true);
		$emailSubject = Q_Config::get('Awards', 'transactional', 'startedSubscription', 'subject', false);
		$emailView = Q_Config::get('Awards', 'transactional', 'startedSubscription', 'body', false);
		...
		if ($mobileView = Q_Config::get('Awards', 'transactional', 'startedSubscription', 'sms', false)) {
	...
	if ($publisher->emailAddress) {
		...
		$email->retrieve(true);
		$emailSubject = Q_Config::get('Awards', 'transactional', 'startSubscription', 'subject', false);
		$emailView = Q_Config::get('Awards', 'transactional', 'startSubscription', 'body', false);
		if ($emailSubject !== false and $emailView) {
			$email->sendMessage($emailSubject, $emailView, $fields);
		}
		...
		$mobile->retrieve(true);
		if ($mobileView = Q_Config::get('Awards', 'transactional', 'startSubscription', 'sms', false)) {
			$mobile->sendMessage($mobileView, $fields);
		}
	}
}