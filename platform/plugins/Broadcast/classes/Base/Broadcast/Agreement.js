 ...
 * Instead, change the Broadcast/Agreement.js file.
 *
 * @module Broadcast
 */
...
function Base (fields) {
	 ...
	 * @type string
	 */
	this.className = "Broadcast_Agreement";
}
...
Base.table = function (withoutDbName) {
	if (Q.Config.get(['Db', 'connections', 'Broadcast', 'indexes', 'Agreement'], false)) {
		return new Db.Expression((withoutDbName ? '' : '{$dbname}.')+'{$prefix}agreement');
	} else {
		var conn = Db.getConnection('Broadcast');
		var prefix = conn.prefix || '';
		var tableName = prefix + 'agreement';
 ...
 * @throws {Error} An exception is thrown if 'value' is not string or is exceedingly long
 */
Base.prototype.beforeSet_details_json = function (value) {
		if (value instanceof Db.Expression) return value;
		if (typeof value !== "string" && typeof value !== "number")
			throw new Error('Must pass a string to '+this.table()+".details_json");
		if (typeof value === "string" && value.length > 255)
			throw new Error('Exceedingly long value being assigned to '+this.table()+".details_json");
		return value;
};
...
Base.prototype.beforeSave = function (value) {
	var fields = ['userId','publisherId','streamName','platform','details_json'], i;
	if (!this._retrieved) {
