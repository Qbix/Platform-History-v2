...
Q.Tool.define("Assets/NFT/preview", ["Streams/preview"], function(options, preview) {
    ...
    previewState.imagepicker = {
        showSize: Assets.NFT.icon.defaultSize,
        save: "NFT/icon"
    };
    ...
    var pipe = Q.pipe(["stylesheet", "text", "interests", "likes"], function (params, subjects) {
        ...
        previewState.onRefresh.add(function (stream) {
            ...
            tool.network = Assets.bockchain.networks.filter(obj => { return obj.chainId === tool.chainId })[0];
    ...
    Q.addStylesheet("css/tools/NFT/preview.css", pipe.fill('stylesheet'), { slotName: 'Assets' });
...

{
    ...
    composer: function () {
        ...
        Q.each(Assets.blockchain.networks, function (i, network) {
        ...
        var _openDialog = function () {
            Q.Dialogs.push({
                ...
                template: {
                    ...
                    fields: {
                        networks: networks,
                        currencies: Assets.blockchain.currencies.map(a => a.symbol),
                ...
                onActivate: function (dialog) {
                    ...
                    $onMarketPlace.click(function() {
                        $(".Assets_nft_form_details", dialog).attr("data-active", $onMarketPlace.prop('checked'));
                    });
                    ...
                    $("button[name=save]", dialog).on(Q.Pointer.fastclick, function (event) {
                        ...
                        var network = Assets.blockchain.networks.filter(obj => { return obj.chainId === chainId })[0];
                        var currencySymbol = $("select[name=currency]", dialog).val();
                        var currency = {};
                        Q.each(Assets.blockchain.currencies, function (i, c) {
...
`<div class="Assets_nft">
    <form>
        ...
        <div class="Assets_nft_form_group SelectNetwork">
