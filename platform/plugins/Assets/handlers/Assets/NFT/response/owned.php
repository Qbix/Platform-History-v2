...
function Assets_NFT_response_owned ($params) {
	...
	foreach ($chains as $chain) {
		...
		if ($tokensByOwner || $getNftsByOwner) {
			...
			for ($i = 0; $i < $tokens; $i++) {
				$tokenId = (int)Users_Web3::execute($pathABI, $chain["contract"], "tokenOfOwnerByIndex", array($accountAddress, $i), $chain["chainId"], true, $glob["Assets_NFT_response_owned"]["secondsInYear"]);
				if ($_Assets_NFT_response_owned_json($tokenId, $chain, $ABI, $tokenJSON, $countNFTs) === false) {
					break;
				}
			}
