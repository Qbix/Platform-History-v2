 ...
 * Instead, change the Assets_Credits.php file.
 *
 * @module Assets
 */
/**
 * Base class representing 'Credits' rows in the 'Assets' database
 * @class Base_Assets_Credits
 * @extends Db_Row
 *
 ...
 * @param {string} [$fields.fromPublisherId] defaults to null
 * @param {string} [$fields.fromStreamName] defaults to null
...
abstract class Base_Assets_Credits extends Db_Row
{
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Assets', 'indexes', 'Credits', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'credits');
		} else {
			$conn = Db::getConnection('Assets');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'credits';
  			if (!$with_db_name)
  				return $table_name;
...
return array (
  ...
  3 => NULL,
	...
	function beforeSet_fromUserId($value)
	{
		...
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".fromUserId");
		if (strlen($value) > 31)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".fromUserId");
		return array('fromUserId', $value);			
	}
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_fromUserId()
	{
...
return array (
  ...
  3 => NULL,
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_toUserId()
	{
