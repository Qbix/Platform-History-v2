/**
 * Autogenerated base class representing connected rows
 * in the Assets database.
 *
 * Don't change this file, since it can be overwritten.
 * Instead, change the Assets/Connected.js file.
 *
 * @module Assets
 */
...
/**
 * Base class representing 'Connected' rows in the 'Assets' database
...
Base.table = function (withoutDbName) {
	if (Q.Config.get(['Db', 'connections', 'Assets', 'indexes', 'Connected'], false)) {
		return new Db.Expression((withoutDbName ? '' : '{{dbname}}.')+'{{prefix}}connected');
	} else {
		var conn = Db.getConnection('Assets');
		var prefix = conn.prefix || '';
		var tableName = prefix + 'connected';
 ...
 * @type string
 */
Base.prototype.className = "Assets_Connected";
 ...
 * Method is called before setting the field and verifies if value is string of length within acceptable limit.
 * Optionally accept numeric value which is converted to string
 * @method beforeSet_accountId
 * @param {string} value
 * @return {string} The value
 * @throws {Error} An exception is thrown if 'value' is not string or is exceedingly long
 */
Base.prototype.beforeSet_accountId = function (value) {
		if (value == null) {
			value='';
		}
		if (value instanceof Db.Expression) return value;
		if (typeof value !== "string" && typeof value !== "number")
			throw new Error('Must pass a String to '+this.table()+".accountId");
		if (typeof value === "string" && value.length > 255)
			throw new Error('Exceedingly long value being assigned to '+this.table()+".accountId");
		return value;
};

	/**
	 * Returns the maximum string length that can be assigned to the accountId field
	 * @return {integer}
	 */
Base.prototype.maxSize_accountId = function () {

		return 255;
};

	/**
	 * Returns schema information for accountId column
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
Base.column_accountId = function () {
...
Base.prototype.beforeSet_refreshToken = function (value) {
		...
		if (typeof value === "string" && value.length > 255)
			throw new Error('Exceedingly long value being assigned to '+this.table()+".refreshToken");
		return value;
};

	/**
	 * Returns the maximum string length that can be assigned to the refreshToken field
	 * @return {integer}
	 */
Base.prototype.maxSize_refreshToken = function () {

		return 255;
};

	/**
	 * Returns schema information for refreshToken column
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
