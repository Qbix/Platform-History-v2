...
abstract class Assets extends Base_Assets
{
	...
	{
		$leaders = Assets_Earned::select('ern.app AS app, CURDATE() AS day_calculated, ern.userId AS userId, SUM(bdg.points) AS points', 'ern')
				->join(Assets_Badge::table().' AS bdg', array('ern.badge_name'=>'bdg.name', 'ern.app'=>'bdg.app'))
	...
	static function getTotalPoints($userId, $app)
	{
		...
		$totalPoints = Assets_Earned::select('SUM(bdg.points) AS total_points', 'ern')
				->join(Assets_Badge::table().' AS bdg', array('ern.badge_name'=>'bdg.name', 'ern.app'=>'bdg.app'))
				->where($whereArray)
				->fetchDbRows();
