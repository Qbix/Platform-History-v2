...
class Zend_Http_Client
{
    ...
    protected function _openTempStream()
    {
        ...
        if(!is_string($this->_streamName)) {
            ...
            $this->_streamName = tempnam(isset($this->config['stream_tmp_dir'])?$this->config['stream_tmp_dir']:sys_get_temp_dir(),
                 'Zend_Http_Client');
        }
        ...
        if (false === ($fp = @fopen($this->_streamName, "w+b"))) {
                ...
                require_once 'Zend/Http/Client/Exception.php';
                throw new Zend_Http_Client_Exception("Could not open temp file {$this->_streamName}");
        }
