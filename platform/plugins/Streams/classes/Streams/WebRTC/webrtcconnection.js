...
class WebRtcConnection extends Connection {
  constructor(id, options = {}) {
    ...
    let connectionTimer = options.setTimeout(() => {
        console.log('connectionTimer onIceConnectionStateChange', peerConnection.iceConnectionState)
    ...
    const onIceConnectionStateChange = () => {
        console.log('onIceConnectionStateChange', peerConnection.iceConnectionState, connectionTimer)
      if (peerConnection.iceConnectionState === 'connected'
        || peerConnection.iceConnectionState === 'completed') {
        if (connectionTimer) {
            console.log('onIceConnectionStateChange connectionTimer', connectionTimer)
    ...
    this.applyOffer = async offer => {
        var description = new RTCSessionDescription({type: offer.type, sdp:offer.sdp});
        return await peerConnection.setRemoteDescription(description).then(function () {
            return peerConnection.createAnswer()
                .then(function(answer) {
                    return peerConnection.setLocalDescription(answer).then(function () {
                        console.log('offerReceived: answer created: sending');
    ...
    this.close = () => {
        console.log('close webrtcconnection')
