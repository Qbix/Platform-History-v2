...
//		}
	 ...
	 *  with empty userId, which is accessed whenever someone makes a request with the
	 *  "Q.Streams.token" querystring parameter containing the token.
	 *  This Streams_Invite is exported under "invite" key of the returned data.
	 *  See the Streams/before/Q_objects.php hook for more information.
	 *  You can also pass an array with two keys here, named "token" and "Q.sig"
	 ...
	 * @method invites
	 * @static
	 * @param {integer} $count The number of invites to create
	 * @param {array} [$tokens] Optionally pass an array of strings,
	 *  each containing a predefined token. Or, an array of arrays with
	 *  "token" and "Q.sig" keys (the function will check the signatures).
	 * @param {array} [$signatures] If tokens are passed, pass signatures
	 */
	static function invites($count, array $tokens = array(), $options = array())
	{
		$maxPerCall = Q_Config::get('Streams', 'invites', 'maxPerCall', 100);
		if ($count > $maxPerCall) {
			throw new Q_Exception_WrongValue(array(
				'field' => 'count',
				'range' => "< $maxPerCall",
		...
		for ($i = 0; $i < $count; ++$i) {
			...
			$t = isset($tokens[$i]) ? $tokens[$i] : null;
			$token = null;
			if (is_array($t)) {
				Q_Valid::requireFields(array('token', 'Q.sig'), $t, true);
				if (!Q_Valid::signature(true, $t)) {
