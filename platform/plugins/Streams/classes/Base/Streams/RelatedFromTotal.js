 ...
 * Instead, change the Streams/RelatedFromTotal.js file.
 *
 * @module Streams
 */
...
Base.table = function (withoutDbName) {
	if (Q.Config.get(['Db', 'connections', 'Streams', 'indexes', 'RelatedFromTotal'], false)) {
		return new Db.Expression((withoutDbName ? '' : '{$dbname}.')+'{$prefix}related_from_total');
	} else {
		var conn = Db.getConnection('Streams');
		var prefix = conn.prefix || '';
		var tableName = prefix + 'related_from_total';
 ...
 * @type string
 */
Base.prototype.className = "Streams_RelatedFromTotal";
...
Base.prototype.beforeSet_relationType = function (value) {
		...
		if (value instanceof Db.Expression) return value;
		if (typeof value !== "string" && typeof value !== "number" && !(value instanceof Buffer))
			throw new Error('Must pass a String or Buffer to '+this.table()+".relationType");
		if (typeof value === "string" && value.length > 255)
			throw new Error('Exceedingly long value being assigned to '+this.table()+".relationType");
		return value;
};
 ...
 * @throws {Error} An exception is thrown if 'value' is not string or is exceedingly long
 */
Base.prototype.beforeSet_toStreamType = function (value) {
		if (value == null) {
			value='';
		}
		if (value instanceof Db.Expression) return value;
		if (typeof value !== "string" && typeof value !== "number" && !(value instanceof Buffer))
			throw new Error('Must pass a String or Buffer to '+this.table()+".toStreamType");
		if (typeof value === "string" && value.length > 255)
			throw new Error('Exceedingly long value being assigned to '+this.table()+".toStreamType");
		return value;
};
	 ...
	 * @return {integer}
	 */
Base.prototype.maxSize_toStreamType = function () {
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
Base.column_toStreamType = function () {
 ...
 */
Base.prototype.beforeSet_relationCount = function (value) {
		if (value instanceof Db.Expression) return value;
		value = Number(value);
		if (isNaN(value) || Math.floor(value) != value) 
			throw new Error('Non-integer value being assigned to '+this.table()+".relationCount");
		if (value < -9.2233720368548E+18 || value > 9223372036854775807)
			throw new Error("Out-of-range value "+JSON.stringify(value)+" being assigned to "+this.table()+".relationCount");
		return value;
};
 ...
 * @return {integer}
 */
Base.prototype.maxSize_relationCount = function () {
...
Base.prototype.beforeSave = function (value) {
	var fields = ['fromPublisherId','fromStreamName','relationType','toStreamType'], i;
	if (!this._retrieved) {
