 ...
 * Instead, change the Streams/RelatedToTotal.js file.
 *
 * @module Streams
 */
 ...
 * @param {String|Buffer} [fields.fromStreamType] defaults to ""
...
Base.table = function (withoutDbName) {
	if (Q.Config.get(['Db', 'connections', 'Streams', 'indexes', 'RelatedToTotal'], false)) {
		return new Db.Expression((withoutDbName ? '' : '{{dbname}}.')+'{{prefix}}related_to_total');
	} else {
		var conn = Db.getConnection('Streams');
		var prefix = conn.prefix || '';
		var tableName = prefix + 'related_to_total';
 ...
 * @type string
 */
Base.prototype.className = "Streams_RelatedToTotal";
 ...
 * @throws {Error} An exception is thrown if 'value' is not string or is exceedingly long
 */
Base.prototype.beforeSet_fromStreamType = function (value) {
		if (value == null) {
			value='';
		}
		if (value instanceof Db.Expression) return value;
		if (typeof value !== "string" && typeof value !== "number" && !(value instanceof Buffer))
			throw new Error('Must pass a String or Buffer to '+this.table()+".fromStreamType");
		if (typeof value === "string" && value.length > 255)
			throw new Error('Exceedingly long value being assigned to '+this.table()+".fromStreamType");
		return value;
};
	 ...
	 * @return {integer}
	 */
Base.prototype.maxSize_fromStreamType = function () {
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
Base.column_fromStreamType = function () {
...
Base.prototype.beforeSave = function (value) {
	var fields = ['toPublisherId','toStreamName','relationType','fromStreamType'], i;
	if (!this._retrieved) {
