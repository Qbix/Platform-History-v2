...
(function (Q, $) {
		...
		invite: function (publisherId, streamName, callback, options) {
			...
			var pipe = Q.pipe(['stream', 'text'], function () {
				Q.Template.render(o.templateName, {
					...
					to: text.to.interpolate({
						"Stream Title": stream.fields.title
					}),
					go: text.go,
					placeholder: text.placeholder,
					orInvite: text.orInvite,
					QR: text.QR.interpolate({
						ClickOrTap: Q.getObject("Q.info.isTouchscreen") ? 'Tap' : 'Click'
					}),
					email: text.byEmail,
					text: text.byText,
	...
	Streams.invite = function (publisherId, streamName, options, callback) {
				...
				Q.each(rsd.userIds, function (i, userId) {
					...
					switch (identifierType) {
						case 'userId':
							break;
						case 'email':
							emailAddresses.push(identifier);
							break;
						case 'mobile':
							mobileNumbers.push(identifier);
							break;
						case 'facebook':
		...
		function _generateInviteURL() {
			...
			return Q.req(o.uri, ['data', 'stream'], function (err, response) {
							...
							Q.Dialogs.push({
								...
								onClose: function () {
									Q.Dialogs.push({
										...
										apply: true,
										content: '<div class="Streams_invite_photo_dialog">' +
				...
				xhr: {
					sync: true
				} // so we can open the window on mobile browsers
			});
		}
	...
	Message.wait = function _Message_wait(publisherId, streamName, ordinal, callback, options) {
		...
		function _tryLoading() {
			...
			if (ordinal < 0) {
				Message.get.forget(publisherId, streamName, {
					min: latest + 1,
					max: ordinal
				});
			}
			return Message.get(publisherId, streamName, {
					min: latest + 1,
					max: ordinal
				},
			...
			for (var i = 0, l = show.length; i < l; ++i) {
				...
				switch (s) {
					...
					case 'u':
						...
						break;
					case 'fu':
						parts.push(fn2 ? fn2 : u2);
						break;
					case 'lu':
						parts.push(ln2 ? ln2 : u2);
						break;
					case 'flu':
					default:
	...
	function prepareStream(stream) {
		...
		try {
			stream.pendingAttributes = stream.attributes = stream.fields.attributes ? JSON.parse(stream.fields.attributes) : {};
		} catch (e) {
	...
	Q.onInit.add(function _Streams_onInit() {
		...
		if (Users.loggedInUser) {
			...
			Q.Streams.Stream.onFieldChanged(
				...
				Users.loggedInUser.id, "Streams/user/lastName", "content"), key, key).or(Q.Streams.Stream.onFieldChanged(
				Users.loggedInUser.id, "Streams/user/username", "content"), key, key).debounce(50, false, key).set(_updateDisplayName, 'Streams');
		}
					...
					Q.Template.render(templateName, params,
						function (err, html) {
							...
							Q.Dialogs.push({
								...
								onActivate: {
									'Streams.completeInvited': function _Streams_completeInvited() {
