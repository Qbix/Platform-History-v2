...
},
	{
		refresh: function () {
			...
			Q.Template.render('Streams/interests', {
				...
				placeholder: state.filter || tool.text.interests.Filter || '',
				all: all
			}, function (err, html) {
				$te.html(html);
				...
				Streams.Interests.load(state.communityId, function () {
					...
					pipe.add(waitFor, 1, function (params, subjects) {
						...
						if (!Q.isEmpty(otherInterests)) {
							for (normalized in otherInterests ) {
								...
								if (!$other.length) {
									var otherText = tool.text.interests.Other;
					...
					var trySynonyms = state.trySynonyms || tool.text.interests.trySynonyms;
					var $unlistedTitle;
