                    ...
                    var videoComposer = (function () {
                        ...
                        function addSource(newSource, successCallback, failureCallback) {
                            ...
                            if(newSource.sourceType == 'webrtcGroup') {
                                ...
                                for (let i = _activeScene.removedWebrtcSources.length - 1; i >= 0; i--) {
                                    let source = _activeScene.removedWebrtcSources[i];
                        ...
                        function updateWebRTCLayout(webrtcGroupSource, layoutName, startAsEmpty) {
                            ...
                            for(let v in participants) {
                                ...
                                if(vTracks.length != 0) {
                                    ...
                                    for (let s in vTracks) {
                                        ...
                                        let pendingAudioTracks = tracksToAdd.filter(function (t) {
                                            return t.kind == 'audio' ? true : false;
                                        })
                                        ...
                                        if(!trackCurrentlyRendered) {
                                            ...
                                            if((audioOnly || notScreensharingVideo) && currentlyRenderedAudioTracks.length == 0 && currentlyRenderedVideoTracks.length == 0) {
                                                ...
                                                if(!vTracks[s].screensharing || (vTracks[s].screensharing && currentlyRenderedVideoTracks.length != 0 && currentlyRenderedAudioTracks.length != 0)) {
                                                    ...
                                                    var replaceAudioTrack = function (audioTrackStream) {
                                                        audioTrackStream.kind = 'video';
                                                        audioTrackStream.track = vTracks[s];
                                                        audioTrackStream.mediaStream = vTracks[s].stream;
                                                        audioTrackStream.htmlVideoEl = vTracks[s].trackEl;
                                                        audioTrackStream.screenSharing = false;
                                                        audioTrackStream.name = audioTrackStream.name.replace(' (screen)', '');
                                                    }
                                                    for(let  z = renderedTracks.length - 1; z >= 0 ; z--){
                                                        ...
                                                        if(renderedTracks[z].kind == 'audio' && !vTracks[s].screensharing) {
                                                            log('updateWebRTCCanvasLayout else REPLACE AUDIO TRACK')
                                                            replaceAudioTrack(renderedTracks[z]);
                                                            replacedAudioTrack = true;
                                                            break;
                                                        }
                                                    }

                                                    if(!replacedAudioTrack && pendingAudioTracks.length != 0) {
                                                        for (let p in pendingAudioTracks) {
                                                            if(pendingAudioTracks[p].kind == 'audio' && !vTracks[s].screensharing) {
                                                                log('updateWebRTCCanvasLayout else REPLACE PENDING AUDIO TRACK')
                                                                replaceAudioTrack(pendingAudioTracks[p]);
                                                                replacedAudioTrack = true;
                                                                break;
                                                            }
                                                        }
                            ...
                            for(let d = _activeScene.webrtcSources.length - 1; d >= 0 ; d--){
                                ...
                                function remove() {
                                    ...
                                    _activeScene.removedWebrtcSources.push(removedSource);
