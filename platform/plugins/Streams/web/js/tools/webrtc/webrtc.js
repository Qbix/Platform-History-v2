...

        {
            ...
            screensRendering: function () {
                ...
                var fitScreenToVideo = function (videoEl, screen) {
                    console.log('LOADED loadedmetadata 1', remoteStreamEl, e.target, e.target.videoHeight, e.target.videoWidth);
                    console.log('LOADED loadedmetadata 2', track.parentScreen.screenEl.style.width, track.parentScreen.screenEl.style.height);

                    if(track.parentScreen.screenEl.style.width != '' || track.parentScreen.screenEl.style.height != '') return;
                    console.log('LOADED loadedmetadata 3', e.target, e.target.videoHeight, e.target.videoWidth);

                    console.log('LOADED loadedmetadata 4', e.target, e.target.videoHeight, e.target.videoWidth);
                    track.parentScreen.width = e.target.videoWidth;
                    track.parentScreen.height = e.target.videoHeight;

                    var videoCon = track.parentScreen.videoCon;
                    var elRect = videoCon.getBoundingClientRect();
                    var nameElRect = track.parentScreen.nameEl.getBoundingClientRect();

                    var ratio0 = remoteStreamEl.videoWidth / remoteStreamEl.videoHeight;
                    console.log('loadedmetadata 5 nameElRect', track.parentScreen.nameEl, nameElRect.width, nameElRect.height);
                    console.log('loadedmetadata 6', elRect.height, elRect.width,  e.target.videoHeight,  e.target.videoWidth);
                    ...
                    if (ratio0 < 1) {
                        ...
                        remoteStreamEl.parentNode.style.flexDirection = 'column';
                    } else {
                        console.log('loadedmetadata if1 else');
