(function (Q, $, window, undefined) {
	...
	Q.text.Q.preview = {
		...
		dialog: {
			title : 'Select Option',
			text: 'Select option to upload an image.'
		},
		image: {
...
Q.Tool.define("Streams/album/preview", "Streams/preview", function(options, preview) {
	...
	if (ps.creatable) {
		...
		ps.creatable.preprocess = function (_proceed) {
			...
			Dialogs.push({
				title: Q.text.Q.preview.dialog.title,
				template: {
					name: "Streams/image/album/select"
				},
				removeOnClose: true,
				className: 'Q_confirm',
				onActivate: function (dialog) {
					// Jquery for Local button on click event
					$(".Q_button_local", dialog).on('click', function () {
						Dialogs.pop();
						_create_Stream();
					});
					// Jquery for Facebook button on click event
					$(".Q_button_fb", dialog).on('click', function () {
						...
						selector_tool = $("<div>").tool("Streams/photoSelector", {
							...
							'onSelect': function (tool, photo, images) {
								...
								toDataUrl(src, function (myBase64) {
									var fields = {
										"title": Q.text.Q.preview.image.facebook
									};
									fields["icon"] = {
...
Q.Template.set('Streams/album/preview/create',
	...
	+ '<div class="Streams_album_preview_title {{titleClass}}">'
	+ '<{{titleTag}} class="Streams_preview_title">'+ Q.text.Q.preview.button.title.add_image +'</{{titleTag}}>'
	+ '</div></div>'
);

Q.Template.set('Streams/image/album/select',
'<div class="Q_messagebox Q_big_prompt">'+
'<p>'+ Q.text.Q.preview.dialog.text +'</p>'+
'<div class="Q_buttons">'+
'<button class="Q_button_local fb_selector-btn">' + Q.text.Q.preview.button.title.file_system +'</button>'+
'<button class="Q_button_fb fb_selector-btn">' + Q.text.Q.preview.button.title.facebook + '</button>' +
'</div>'+
'</div>'
);
